...
class SimulationConfig:
    ...
    # Always show this many H-period returns:
    n_h_periods: int = 40
    ...
    def __post_init__(self):
        ...
        if self.n_paths_predictable < 0:
            raise ValueError("Number of paths cannot be negative")
        
        if self.n_paths_predictable == 0:
            raise ValueError("At least one path type must be requested")
        self.years_insample = self.n_h_periods * self.time_horizon
        self.years_oos = self.time_horizon
...
class PathSimulator:
    ...
    def __init__(self, config: SimulationConfig):
        ...
        self.N_is = self.config.years_insample // self.config.time_horizon   # = 40
        self.T_is = self.N_is * self.config.time_horizon                     # = 40H

        self.N_oos = self.config.years_oos // self.config.time_horizon        # = 1
        self.T_oos = self.N_oos * self.config.time_horizon                    # = H
    ...
    def run(self) -> Dict:
        ...
        if self.config.n_paths_predictable > 0:
            ...
            signal_pred_actual_selected = signal_pred_actual[:, idx_pred]
